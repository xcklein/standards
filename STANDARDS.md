...
| [ADR-AWS-0002](aws/ADR-AWS-0002-use-cdk-nag.md) | Use CDK Nag | Every CDK app entry point must apply `AwsSolutionsChecks` app-wide, via `Aspects.of(app).add(...)` or `Validations.of(app).addPlugins(...)`. The CDK synth step must fail on unsuppressed violations. All suppressions and acknowledgements must include a `reason`. |
| [ADR-AWS-0003](aws/ADR-AWS-0003-tag-all-resources.md) | Tag All Resources | All AWS resources must be tagged with `x:repo`, `x:service`, and `x:env` using the CDK `Tags` API, applied at either the app or the stack level. Additional tags may be added as needed. |

## TypeScript

| ID | Title | Directive |
|----|-------|-----------|
| [ADR-TS-0001](typescript/ADR-TS-0001-use-linter.md) | Use ESLint | All TypeScript projects must use ESLint for linting. Biome must not be used for linting. Every project must include a flat config at the project root, under any filename ESLint resolves natively (`eslint.config.js`, `.mjs`, `.cjs`, `.ts`). |
| [ADR-TS-0002](typescript/ADR-TS-0002-use-lefthook.md) | Use Lefthook | All TypeScript projects must use Lefthook for Git hooks. A Lefthook config must be present at the project root, in any format Lefthook resolves natively (`.yml`, `.yaml`, `.json`, `.toml`). At minimum it must run `eslint --fix` and `prettier --write` on staged files via a `pre-commit` hook and enforce commit message format via a `commit-msg` hook (see ADR-GIT-0001). |
| [ADR-TS-0003](typescript/ADR-TS-0003-use-esm.md) | Use ESM | All TypeScript projects must use ESM, not CommonJS. `package.json` must include `"type": "module"`. Node-runtime projects use `"moduleResolution": "NodeNext"` (or `"Node16"`) with an explicit extension on relative imports — `.js`, or `.ts` with `rewriteRelativeImportExtensions` — applied consistently; bundler-driven frontend projects use `"moduleResolution": "bundler"` with extensionless imports. |
...
| [ADR-TS-0016](typescript/ADR-TS-0016-avoid-global-variables.md) | Avoid Global Variables | Mutable module-level state (`let`/`var` bindings at module scope, or exported mutable singletons) must not be used. Module-scoped `const` bindings that are genuinely immutable are fine. State that changes over time must be owned by and passed through an explicit carrier — a function parameter, a class instance, a dependency-injection container, or framework-level state — rather than read or written through a shared global binding. |
...
| [ADR-API-0006](api/ADR-API-0006-pagination.md) | Pagination | All collection endpoints whose result set grows with usage must use cursor-based pagination. Offset-based pagination is not permitted. Responses must include a `pagination` object and `Link` headers. Collections bounded by configuration rather than user data (fixed top-N, enums) are exempt if the bound is declared in the schema. |
...
| [ADR-API-0008](api/ADR-API-0008-content-negotiation.md) | Content Negotiation | All API responses must use `Content-Type: application/json`. Error responses must use `Content-Type: application/problem+json`. File uploads must use `multipart/form-data`, and endpoints whose response body is a binary asset must use that asset's own media type. |
...
| ID | Title | Directive |
|----|-------|-----------|
| [ADR-DOCS-0001](documentation/ADR-DOCS-0001-doc-comments.md) | Doc Comments | All exported/public functions, classes, methods, and types must have a documentation comment (JSDoc, Javadoc, or the language's equivalent). Doc comments must describe purpose, parameters, return values, and thrown/rejected errors where applicable. The free-form description should be limited to around a dozen lines; if that's not enough, link a separate supporting document instead. Doc comments must never reference git commits, branches, PRs, or issues — only source-controlled documentation (cross-repo references allowed). A doc comment must let the code speak for itself wherever possible, and must not make claims about the wider codebase that can't be verified locally and are likely to go stale. |
| [ADR-DOCS-0002](documentation/ADR-DOCS-0002-inline-comments.md) | Inline Comments | Inline code comments must be used sparingly. They are permitted only to explain unexpected or non-obvious behavior, or to explicitly note the deliberate absence of something (e.g., a `catch` block with no `throw`). Comments must not restate what the code already says. An inline comment should be limited to about three lines; if that's not enough, link a separate supporting document instead. Inline comments must never reference git commits, branches, PRs, or issues — only source-controlled documentation (cross-repo references allowed). An inline comment must let the code speak for itself wherever possible, and must not make claims about the wider codebase that can't be verified locally and are likely to go stale. |
...
| [ADR-UI-0008](ui/ADR-UI-0008-use-lucide.md) | Use Lucide | All icons must come from `lucide-react`. Icon imports must use the `XxxIcon` suffixed export (e.g. `DownloadIcon`) — never the deprecated unsuffixed alias (e.g. `Download`). Enforcing this with a lint rule is recommended over relying on convention or code review alone. |
